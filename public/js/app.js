class RTSPWebViewer {
    ...
    bindEvents() {
        ...
        document.getElementById('stop-stream').addEventListener('click', () => this.stopCurrentStream());
        document.getElementById('refresh-streams').addEventListener('click', () => this.loadActiveStreams());
    ...
    async startStream() {
        ...
        try {
            ...
            this.logMessage(`Error al iniciar stream ${streamId}: ${error.message}`, 'error');
        } finally {
    ...
    async stopStream(streamId) {
        ...
        try {
            ...
            this.logMessage(`Error al detener stream ${streamId}: ${error.message}`, 'error');
        }
    }
    ...
    renderStreams() {
        ...
        const streamsHTML = Array.from(this.activeStreams.values()).map(stream => {
    ...
    async playStream(streamId, hlsUrl) {
        ...
        try {
            ...
            this.logMessage(`Error al reproducir stream ${streamId}: ${error.message}`, 'error');
    ...
    handleStreamError(streamId, error) {
        ...
        this.logMessage(`Error en stream ${streamId}: ${error.type || error.message}`, 'error');

        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            this.reconnectAttempts++;
            this.showToast(`Reintentando conexión (${this.reconnectAttempts}/${this.maxReconnectAttempts})...`, 'warning');
            ...
            this.showToast(`Stream ${streamId} falló tras ${this.maxReconnectAttempts} intentos`, 'error');
            this.stopCurrentPlayback();
            this.reconnectAttempts = 0;
        }
    }
